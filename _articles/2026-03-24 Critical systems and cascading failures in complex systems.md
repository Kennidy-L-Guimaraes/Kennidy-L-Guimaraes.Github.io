...
references:
  cascading:
    text: "Cascading failures in complex networks"
    url: "https://academic.oup.com/comnet/article/8/2/cnaa013/5849333"

  springer: 
    text: "Reliability Evaluation of Machine Center Components Based on Cascading Failure Analysis"
    url: "https://link.springer.com/article/10.1007/s10033-017-0144-y"

  sciencedirect:
    text: "Failure dependence and cascading failures: A literature review and research opportunities"
...
The criticality of a system can be analyzed by considering two fundamental principles: the inevitability of failures over time and the tendency of systems to fail at their most fragile points {% include ref.html id="cascading" %}.
...
We know that failures do not occur in isolation, but rather progressively and in chain. An initial failure can trigger a sequence of events that propagate through the system, amplifying its original impact {% include ref.html id="cascading" %} {% include ref.html id="springer" %}.
...
A problem that starts at $A_1$ can lead to an error in $A_2$, which in turn has its own dependencies, propagating the failure to $A_3$ and so on, until the system is interrupted or unforeseeable side effects occur {% include ref.html id="springer" %}.
<span class="destaque-bloco">
