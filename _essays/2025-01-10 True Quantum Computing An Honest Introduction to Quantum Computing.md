...
references:
  ...
  - "Oliveira, Maria Luiza de. (2018). Deviations from concepts of quantum theory through the bricolage of non-scientists. Master's thesis — State University of Campinas. https://hdl.handle.net/20.500.12733/1634823"         # [2]
...
As mentioned earlier, atoms and particles are found in everything around us — and this is not limited to the present; past events do not constrain quantum nature. In 1935, Einstein, Boris Podolsky, and Nathan Rosen proposed the viability of maintaining a correlated configuration for two particles {% include ref.html n=6 %}. These "twin particles" maintained correlated properties: when one particle changed, the other would follow suit.
...
All particles that exist in the universe can be divided into two great families: **Bosons** and **Fermions**. The difference between them lies in their spin {% include ref.html n=11 %}.
...
Understand it from this angle: if a classical computer, using Grover's algorithm, takes $O(n)$ to search an unsorted list, a quantum computer using the same algorithm takes approximately $O(\sqrt{n})$ — because, in quantum superposition, all possible states can be available simultaneously {% include ref.html n=10 %}:
...
The Cloud Security Alliance (CSA) estimates that by 2030, quantum computers may be capable of breaking current cryptographic algorithms {% include ref.html n=13 %}. Projections indicate that around 200,000 qubits would be needed to break RSA-2048 in 24 hours — a scenario far from the current capacity of hundreds to a few thousand physical qubits.
